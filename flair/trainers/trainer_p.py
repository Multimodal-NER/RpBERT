        ...
        try:
            ...
            for self.epoch in range(self.epoch + 1, max_epochs + 1):
                ...
                if (
                    ...
                    if anneal_with_restarts:
                        self.model.module.load_state_dict(
                            self.model.module.load(base_path / "best-model.pt").state_dict()
                        )
                    if anneal_with_prestarts:
                        self.model.module.load_state_dict(
                            self.model.module.load(base_path / "pre-best-model.pt").state_dict()
                        )
                ...
                if (
                    ...
                    self.model.module.save(base_path / "pre-best-model.pt")
                    self.model.module.load_state_dict(current_state_dict)

            # if we do not use dev data for model selection, save final model
            if save_final_model and not param_selection_mode:
                self.model.module.save(base_path / "final-model.pt")
