...
class SequenceTagger(flair.nn.Model):
    def __init__(
        ...
        self.attn = torch.nn.MultiheadAttention(hidden_size * 2, num_heads=8, dropout=0.1)
        if self.use_crf:
    ...
    def forward(self, sentences: List[Sentence]):
        ...
        if self.use_rnn:
            ...
            sentence_tensor = self.attn(sentence_tensor, sentence_tensor, sentence_tensor)[0]
            print(sentence_tensor.shape)
            if self.use_dropout > 0.0:
                sentence_tensor = self.dropout(sentence_tensor)
